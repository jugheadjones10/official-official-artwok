...
class SearchListingViewModel : ViewModel() {
    ...
    val products: LiveData<PagedList<SearchListing>> = LivePagedListBuilder(dataSourceFactory, pagedListConfig).build()
    val searchBox = SearchBoxConnectorPagedList(searcher, listOf(products))

    val adapterProduct = SearchListingsAdapter()
